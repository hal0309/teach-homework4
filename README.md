...
### 1. APIへのアクセス
今回は、犬の画像をランダムで取得できるAPIを使用します。
APIのURLは以下の通りです。

```
https://random.dog/woof.json?include=jpeg
```

これにアクセスすると、以下のようなレスポンスが帰ってきます。
    
```json
{
  "fileSizeBytes": 234776,
  "url": "https://random.dog/8c9dd457-5907-4fd4-a825-f8c576fe1284.jpeg"
}
```

postman等を使用して、APIにアクセスし、画像のURLが取得できることを確認してください。  
また、取得したURLをコピペして、ブラウザでアクセスすると、画像が表示されることを確認してください。

### 2. 犬の表示ページの確認
`dog.html`と`Dog.js`というファイルを作成しました。  
dogのページにアクセスすると、以下のように一匹の犬が表示されていることを確認してください。

また、`Dog.js`内で、APIにアクセスして結果をconsoleに出力する処理を行っています。  
`Dog.js`の内容を確認すると共に、
dogのページにアクセスした際に、consoleにAPIの結果が表示されていることを確認してください。

![image](https://github.com/user-attachments/assets/b6b6c57a-5cf7-4985-b1dc-9a03a7ed15a8)


### 3. 犬のランダム表示
現状、dogページにアクセスしても、毎回同じ犬の画像が表示されています。  
`Dog.js`を編集することで、毎回異なる犬の画像が表示されるようにしてください。

ヒント  
`callDogApi()`関数内で、console出力を呼び出しているところを、画像のURLを取得する処理に変更してください。

### 4. ファイルサイズの表示
APIのレスポンスには、画像のファイルサイズが含まれています。
`Dog.js`を編集して、画像を読みこんんだ際にファイルサイズをアラートで表示してください。

![image](https://github.com/user-attachments/assets/85f67114-71da-4211-bd75-f5927918acb6)


### 5. 複数画像の取得
最初1枚だった犬の画像が、「犬、たくさん来い」ボタンを作成し、押すことで沢山表示されるようにしてください。

犬を沢山呼び出すリクエストは以下のURLを使用してください。
```
https://random.dog/doggos?include=jpeg
```
上記を呼び出すと、配列で複数の画像の名称が返ってきます。
``` json
[
...
この画像名はそのままでは表示出来ないので`https://random.dog/` と結合して、画像のURLを作成し、表示してください。

![image](https://github.com/user-attachments/assets/d096aaed-1485-4fd9-afb2-045f2c2dd635)
...
### 1. 完成イメージ
今回の課題では、以下のような日誌を記録できるページを作成します。

日誌は日付と評価(1-5の5段階)で記録します。
工程に関して特に指示はありませんが、後述する要件を満たすようにしてください。

提出の際はスクリーンショットを添付してください。


![image](https://github.com/user-attachments/assets/990d28b4-17c2-4970-90d5-4805ef82d6f4)

### 2. ファイル構成の要件
- htmlファイルは`diary.html`を作成し、`index.html`と同じディレクトリに配置してください。
- cssファイルは`style.css`を読み込んでください。
- JavaScriptファイルは`Diary.js`を作成し、読み込んでください。

### 3. 外見の要件
- ページタイトルは`日誌`としてください。
- 「月」「日」「評価」の3つの数字入力欄と保存ボタンを作成してください。
- 日誌一覧の下の部分に、保存した日誌が追加されるスペースを作成してください。
  - 参考画像は3回保存した状態です。保存ボタンを押す前は何も表示されません。
- その他ヘッダー等、できるだけ参考画像に近い配置になるようにしてください。
  - cssを用いる必要はありません。

### 4. 機能の要件
- 保存ボタンを押した際、以下の順にエラー処理を行い、エラーメッセージを**アラートで**表示してください。
  - 月、日、評価のいずれかが空欄の場合 -> `全ての項目を入力してください。`
  - 月、日、評価のいずれかが整数でない場合 -> `日付と評価は整数で入力してください。`
  - 月が1~12の範囲外の場合 -> `月は1から12の間で入力してください。`
  - 日が1~31の範囲外の場合 -> `日は1から31の間で入力してください。`
  - 評価が1~5の範囲外の場合 -> `評価は1から5の間で入力してください。`
- 上記のエラーに当てはまらない場合、入力された日誌が`○月×日の評価: △`として、日誌一覧の下のスペースに追加されるようにしてください。
- 評価`△`は、太字にしてください。
- 保存ボタンを押した後、全ての入力欄の値は空になるようにしてください。
## 2024年7月21日 演習
